...
class NewsPresenter(private val newsViewContract: NewsViewContract) :
    ...
    fun setupContentViewModel(fragmentActivity: FragmentActivity?) {
        ...
        contentViewModel?.items?.observe(newsViewContract.getViewLifecycleOwner(), Observer{ items ->
    ...
    private fun updateSourcePriority(context: Context) {
        ...
        Settings.getInstance(context)
            .setPriority(PREF_INT_NEWS_PRIORITY, Settings.PRIORITY_USER)
    }
}